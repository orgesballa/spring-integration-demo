...
@Configuration
public class ClientAConfig implements ApplicationEventPublisherAware {

    @Value("${clientA.tcp.host}")
    private String host;

    @Value("${clientA.tcp.port}")
    private int port;

    @Value("${clientA.tcp.connection.poolSize}")
    ...
    public ClientAConfig(AbstractByteArraySerializer codec) {
        this.codec = codec;
    }
     ...
     */
    @Bean
    public AbstractClientConnectionFactory clientAConnectionFactory() {
     ...
     */
    @Bean
    public MessageChannel clientAOutboundChannel() {
     ...
     */
    @Bean
    @ServiceActivator(inputChannel = "clientAOutboundChannel")
    public MessageHandler clientAOutboundGateway(
            @Qualifier("clientAConnectionFactory") AbstractClientConnectionFactory clientConnectionFactory) {
