...
@Configuration
public class ClientBConfig implements ApplicationEventPublisherAware {

    @Value("${clientB.tcp.host}")
    private String host;

    @Value("${clientB.tcp.port}")
    private int port;

    @Value("${clientB.tcp.connection.poolSize}")
    ...
    public ClientBConfig(AbstractByteArraySerializer codec) {
        this.codec = codec;
    }
     ...
     */
    @Bean
    public AbstractClientConnectionFactory clientBConnectionFactory() {
     ...
     */
    @Bean
    public MessageChannel clientBOutboundChannel() {
     ...
     */
    @Bean
    @ServiceActivator(inputChannel = "clientBOutboundChannel")
    public MessageHandler clientBOutboundGateway(
            @Qualifier("clientBConnectionFactory") AbstractClientConnectionFactory clientConnectionFactory) {
